...
function Resolve-BackupPath {
    ...
    $preferred = Join-Path $backupsDir "talent_match_db_2026-03-10.sql"
    if (Test-Path -LiteralPath $preferred) {
        return (Resolve-Path -LiteralPath $preferred).Path
    }

    $legacyPreferred = Join-Path $backupsDir "softscale_backup_20251209_225229.sql"
    ...
    Write-Host "No SQL backup found under $backupsDir (*.sql)." -ForegroundColor Red
    exit 1
}
...
function Invoke-SanitizedRestore {
    ...
    try {
        ...
        if ($leaf -ieq "softscale_backup_20251209_225229.sql") {
            # Restore legacy backup (already UTF-8 after normalization) without sanitizing COPY bodies.
