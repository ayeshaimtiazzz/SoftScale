...
class JobRepository:
    ...
    @staticmethod
    def get_available_projects_for_deals(conn, exclude_company_id: int) -> List[Dict[str, Any]]:
        """Get all projects excluding those from the specified company (for pursuing as deals).
        Returns ALL projects from other companies with no additional filtering conditions.
        """
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                ...
                WHERE p.company_id IS NULL OR p.company_id != %s
                ORDER BY p.created_at DESC
            """, (exclude_company_id,))
            rows = cur.fetchall()
            colnames = [desc[0] for desc in cur.description]
